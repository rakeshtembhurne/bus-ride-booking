...
<div className="flex space-x-2">
  <button
    ...
    className={`px-3 py-1 border ${pageIndex === 1 ? "opacity-50 cursor-not-allowed" : ""}`}
  >
    Previous
  </button>
  {Array.from({ length: totalPages }).map((_, index) => {
    ...
    return (
      <button
        ...
        className={`px-3 py-1 border ${pageIndex === page ? "bg-blue-500 text-white" : ""}`}
      >
        {page}
      </button>
    );
  ...
  <button
    ...
    className={`px-3 py-1 border ${pageIndex === totalPages ? "opacity-50 cursor-not-allowed" : ""}`}
  >
    Next
  </button>
</div>
