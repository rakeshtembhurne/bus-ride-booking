...
import { faEye, faEdit, faTrash, faR } from "@fortawesome/free-solid-svg-icons";
...
export function DataTableDemo() {
  ...
  const handleEditF = (fare: Fare) => {
    ...
    const query = new URLSearchParams({
      ...
      price: String(fare.price),  // Ensure price is a string
    }).toString();
    const url = `/dashboard/fare/edit/${fare.id}?${query}`;
    router.push(url);
  };
                  ...
                  <TableCell>
                    <div className="flex space-x-2">
                      <Button
                        variant="link"
                        onClick={() => {
                          setFarename(fare.fromLocation || "");
                          setFarename(fare.toLocation || "");
                          setViewMode(true);
                          setIsOpen(true);
                        }}
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-96 rounded-md bg-white p-6 shadow-md">
            ...
            {viewMode && (
              <>
                <p>
                  <strong>From Location:</strong> {fareName}
                </p>
                <p>
                  <strong>To Location:</strong> {fareName}
                </p>
              </>
            )}
