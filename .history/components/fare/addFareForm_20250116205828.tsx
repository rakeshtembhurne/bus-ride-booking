...
export default function AddFarePage() {
  ...
  const fetchRoutes = async () => {
    try {
      ...
      console.log('Fetched routes:', data); // Ensure data structure is as expected
      setRoutes(data); // Set full route data
    } catch (error) {
      console.error('Error fetching routes:', error);
    }
  };

  const [users, setUsers] = useState<any[]>([]); // Declare setUsers here
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await fetch('/api/userId'); // API endpoint to get users
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        }
        const data = await response.json();
  ...
  const onSubmit = async (data: FareFormValues) => {
    try {
      const user = users[0]; // Assuming the first user is the one you want (adjust as needed)
