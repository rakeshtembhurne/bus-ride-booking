...
export default function AddFarePage() {
  ...
  const onSubmit = async (data: FareFormValues) => {
    try {
      if (!userId) {
        // If there's no user logged in, return early
        return;
      }
      ...
      const fareData = {
        ...
        createdByUserId: userId, // Use the dynamically fetched userId
      };
      ...
      router.push("/fare"); // Redirect to fare page after success
    } catch (error) {
      console.error(error);
    }
  };
                ...
                >
                  ...
                  <option key={route.id} value={route.id}>
                    {route.origin?.name && route.destination?.name
                      ? `${route.origin.name} to ${route.destination.name} - Departs at ${new Date(route.departureTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`
                      : `Route ${route.id}`}
                  </option>
                ))}
                </select>
