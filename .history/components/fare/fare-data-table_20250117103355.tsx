...
export function DataTableDemo() {
  ...
  try {
    ...
    // Map the API response to match the columns and get location names
    const formattedData = fares.map((fare: any) => {
      const fromLocation = locations.find(
        (location) => location.id === fare.fromLocationId
      );
      const toLocation = locations.find(
        (location) => location.id === fare.toLocationId
      );

      return {
        routeId: fare.routeId,
        fromLocationId: fromLocation ? fromLocation.name : fare.fromLocationId,
        toLocationId: toLocation ? toLocation.name : fare.toLocationId,
        price: fare.price,
      };
    });
