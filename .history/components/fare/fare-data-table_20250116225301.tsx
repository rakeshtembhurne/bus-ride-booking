...
export const columns: ColumnDef<Fare>[] = [
  {
    accessorKey: "route",
    header: ({ column }) => <span>{column.id}</span>, // Example of dynamic header content
  ...
  {
    id: "actions",
    header: "Actions", // Can be a direct string or JSX
    cell: ({ row }) => {
    ...
    async function fetchFares() {
      try {
        ...
        if (response.ok) {
          ...
          console.log("Fetched fares:", data); // Log the fetched data
