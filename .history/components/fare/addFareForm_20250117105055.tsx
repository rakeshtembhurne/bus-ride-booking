...
export default function AddFarePage() {
  ...
  const fetchLocations = async () => {
    try {
      const response = await fetch('/api/fare/location'); // API endpoint to fetch locations
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      const data = await response.json();
  ...
  const onSubmit = async (data: FareFormValues) => {
    try {
      ...
      const fareData = {
        routeId: data.route,
        fromLocationId: locations.find((loc) => loc.id === data.fromLocation)?.name || "", // Use location name here
        toLocationId: locations.find((loc) => loc.id === data.toLocation)?.name || "", // Use location name here
