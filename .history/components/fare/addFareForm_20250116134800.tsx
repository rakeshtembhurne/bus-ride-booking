...
export default function AddFarePage() {
  ...
  const onSubmit = async (data: FareFormValues) => {
    try {
      ...
      // Parse the data with the Zod schema, including the userId
      const validatedData = fareSchema.parse({
        routeId: data.route,
        fromLocationId: data.fromLocation,
        toLocationId: data.toLocation,
        price: parseFloat(data.price),
        createdByUserId: userId, // Ensure userId is passed here
      });
    
      console.log("Sending validated data:", validatedData);  // Log here to inspect the data
    
      const response = await axios.post('/api/fare', validatedData);
