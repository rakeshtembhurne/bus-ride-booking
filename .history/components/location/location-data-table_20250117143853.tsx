...
import DeleteLocationPage from '@/app/(protected)/location/delete/page'
export const columns: ColumnDef<{ name: string }>[] = [
  ...
  onClick={async () => {
    ...
    if (confirmation) {
      try {
        // Call the DELETE API route to delete the location
        const response = await fetch(`/api/locations/${row.id}`, {
          method: "DELETE",
        });

        if (response.ok) {
          alert("Location deleted successfully.");
          // Refresh the locations list after deleting the item
          const updatedLocations = locations.filter(location => location.id !== row.id);
