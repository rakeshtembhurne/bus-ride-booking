...
export function DataTableDemo() {
  ...
  const handleEdit = (location: { id: string; name: string }) => {
    ...
    setLocationName(location.name); // Set the current name
    setIsOpen(true);
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const response = await fetch(`/locations/edit/${location.id}`, {
        method: 'PUT',
        body: JSON.stringify({
          name: locationName,
        }),
  ...
  const handleView = (location: { id: string; name: string }) => {
    setViewMode(true);
    setLocationName(location.name); // Set the name of the location manager (or location name)
    setIsOpen(true);
  };
