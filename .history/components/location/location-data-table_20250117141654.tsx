  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex justify-end space-x-2">
        ...
        <Button
          variant="link"
          onClick={() => handleDelete(row.getValue("name"), row.getValue("id"))} // Call the delete handler
        >
...
export function DataTableDemo() {
  const [locations, setLocations] = React.useState<{ name: string, id: string }[]>([]) // Updated state type with 'id'
  ...
  React.useEffect(() => {
    async function fetchLocations() {
      try {
        ...
        if (response.ok) {
          setLocations(data); // Set the locations in state, assuming each location has an 'id' field
        } else {
    ...
    if (confirmation) {
      try {
        ...
        if (response.ok) {
          ...
          setLocations((prevLocations) => prevLocations.filter(location => location.id !== id)); // Update state after deletion
        } else {
          const data = await response.json();
          alert(`Failed to delete ${name}: ${data.error}`);
        }
      } catch (error) {
