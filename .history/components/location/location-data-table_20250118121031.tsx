...
export function DataTableDemo() {
  ...
  const handleEdit = (location: { id: string; name: string }) => {
    ...
    setManagerEmail(location.email || ""); // Set the current email if available
    setIsOpen(true);
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const response = await fetch(`/api/locations/${managerId}`, {
        method: 'PUT',
        body: JSON.stringify({
          name: managerName,
          email: managerEmail,
      ...
      if (response.ok) {
        ...
        setLocations((prevLocations) =>
          prevLocations.map((location) =>
            location.id === managerId
              ? { ...location, name: managerName, email: managerEmail }
              : location
          )
        );
