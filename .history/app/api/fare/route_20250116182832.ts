...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "GET") {
    try {
      ...
      console.log("Fetched routes:", routes);  // Log the raw data before mapping
      ...
      console.log("Formatted routes:", formattedRoutes);  // Log the formatted data

      res.status(200).json(formattedRoutes);
    } catch (error) {
      console.error("Error fetching routes:", error);  // Log any error
