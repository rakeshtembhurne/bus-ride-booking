...
export default function AddFarePage() {
  ...
  const { control, handleSubmit, formState: { errors } } = useForm<FareFormValues>()
  ...
  const onSubmit = async (formData: FareFormValues) => {
    try {
      ...
      const data = {
        ...
        createdByUserId: "user-id", // Set the logged-in user ID here
      };
      ...
      if (response.ok) {
        ...
        console.error('Failed to add fare:', result.error);
      }
    } catch (error) {
