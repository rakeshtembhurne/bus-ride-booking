...
export default function AddFarePage() {
  ...
  const onSubmit = async (data: FareFormValues) => {
    try {
      const response = await axios.post('/api/fare/route', {
        route: data.route,
        fromLocation: data.fromLocation,
        toLocation: data.toLocation,
        price: data.price,
        userId, // Pass the user ID if needed for tracking
      })
      console.log('Fare added successfully:', response.data)
      router.push("/fare") // Redirect to the fare page or wherever necessary
    } catch (error) {
