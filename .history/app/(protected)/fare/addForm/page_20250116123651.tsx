...
export default function AddFarePage() {
  ...
  const onSubmit = async (data: FareFormValues) => {
    try {
      console.log("Form data before validation:", data); // Log the data received from the form
      ...
      const response = await axios.post('/api/fare/route', validatedData);
  
      console.log('Fare added successfully:', response.data); // Log the success response
  
      router.push("/fare");
    } catch (error) {
      console.error('Error adding fare:', error); // Log any error from the API call
